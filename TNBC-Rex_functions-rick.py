...
def cufflinks_merge(fpkm_dir, out_dir, run_name):
	...
	final_df.to_csv(str(out_dir/'rpkm_tables.csv'.format(run_name)), sep=',', index=True)
	np.log2(
...
def run_samples_through_ref_set(file):
	...
	sp.call([
		...
		script,
		file])
	print('rpkm_merge_results file with ref_set+lean results is located in Bioinf_Tools/prnaSeq/ref_set_testing/tables')